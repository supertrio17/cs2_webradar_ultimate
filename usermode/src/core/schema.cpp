...
bool schema::setup()
{
    const std::vector<std::string> modules = { CLIENT_DLL, ENGINE2_DLL };

    for (const auto& module_name : modules)
    {
        const auto type_scope = i::m_schema_system->find_type_scope_for_module(module_name);
        if (!type_scope)
            continue;

        const auto table_size = type_scope->m_hash_classes().size();
        LOG_INFO("found '%d' schema classes in module '%s'", table_size, module_name.c_str());
