#include "pch.hpp"
 ...
 int c_memory::setup()
 {
        LOG_DEBUG("memory::setup started");
        ...
        LOG_DEBUG("memory::setup found cs2.exe with pid '%u'", this->m_id);
...
std::optional<void*> c_memory::hijack_handle()
{
    ...
    LOG_DEBUG("memory::hijack_handle started for pid '%u'", this->m_id);

    const auto ntdll = GetModuleHandleA("ntdll.dll");
    if (!ntdll)
    {
        LOG_WARNING("memory::hijack_handle failed: ntdll.dll is not loaded");
        return {};
    }
    ...
    {
        LOG_WARNING("memory::hijack_handle failed: NtQuerySystemInformation was not found");
        return {};
    }
    ...
    {
        LOG_WARNING("memory::hijack_handle failed: NtDuplicateObject was not found");
        return {};
    }
    ...
    if (!nt_open_process)
    {
        LOG_WARNING("memory::hijack_handle failed: NtOpenProcess was not found");
        return {};
    }
    ...
    {
        LOG_WARNING("memory::hijack_handle failed: RtlAdjustPrivilege was not found");
        return {};
    }
    ...
    {
        LOG_WARNING("memory::hijack_handle failed: NtQuerySystemInformation returned status '0x%lX'", status);
        cleanup(handle_info, handle.first);
        return {};
    }

    const auto system_handle_info = reinterpret_cast<system_handle_info_t*>(handle_info.data());
    LOG_DEBUG("memory::hijack_handle scanning '%lu' system handles", system_handle_info->m_handle_count);
    for (uint32_t idx = 0; idx < system_handle_info->m_handle_count; ++idx)
    {
        ...
        {
            LOG_DEBUG("memory::hijack_handle duplicated handle for target pid '%u'", this->m_id);
            cleanup(handle_info, handle.first);
    ...
    cleanup(handle_info, handle.first);
    LOG_DEBUG("memory::hijack_handle did not find a reusable handle for pid '%u'", this->m_id);
    return {};
}
